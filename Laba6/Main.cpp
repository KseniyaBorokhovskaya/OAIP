...
void TWindowTree::ShowTree(TTreeView *TreeView, TTreeNode *treeNode, Node *node)
	{
		if (node != NULL) {
			if (node == tree->GetHead()) {
				treeNode = TreeView->Items->Add(NULL, node->Name + "(" + IntToStr(node->WorkmanID) + ")");
			}
			else {
				treeNode = TreeView->Items->AddChild(treeNode, node->Name + "(" + IntToStr(node->WorkmanID) + ")");
			}
			ShowTree(TreeView, treeNode, node->left);
			ShowTree(TreeView, treeNode, node->right);
		}
		else return;
	}
